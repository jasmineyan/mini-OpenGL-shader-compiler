...
Description:
Build a parser(using the Bison parser generator) that accepts the language generated by the MiniGLSL grammar. Parser must implement the "trace parser" functionality (-Tp command-line flag) of the compiler. No semantic analysis or AST construction need to be performed. 

Starter files:
complete implementation of scanner.l as well as a dummy parser.y files.
...
unary_op      -> '!' | '-'
binary_op     -> '&&'|'||'|'=='|'!='|'<'|'<='|'>'|'>='|'+'|'-'|'*'|'/'|'^'
construtor    -> type '(' arguments ')'
function      -> function_name '(' arguments_opt ')'
function_name -> 'dp3'|'lit'|'rsq'
arguments_opt -> arguments | empty string
arguments     -> arguemtns ',' expression | expression
